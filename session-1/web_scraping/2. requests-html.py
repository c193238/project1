...
def extract_information(url):
    ...
    try:
        ...
        response = session.get(url)
        time_tag = response.html.find('span')
        print("Time: ", time_tag[0].text)
        
        first_image = response.html.find('img', first=True)
        if first_image:
            print("First Image source:", first_image.attrs.get('src'))
        else:
        ...
        # temp = response.html.xpath('div.w3-padding-small > b', first=True)
        # print(temp)
...
def main():
    ...
    extract_information('https://businesspostbd.com/economy/white-paper-will-highlight-corruption-issues-not-catch-culprits-debapriya')
